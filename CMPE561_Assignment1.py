...
def evaluation(sentences,original_or_stem):
    ...
    for index_train, index_test in kf.split(sentences):
        ...
        list_target_tags, list_output, list_tag_accuracy = [], [], []
    ...
    final_accuracy_of_tagging_words = accuracy_of_tagging_words/float(number_of_folds)
    final_accuracy_of_tagging_sentences= accuracy_of_tagging_sentences/float(number_of_folds)
    # Calculate tag accuracy
    for i in range(len(final_confusion_matrix)):
        for j in range(len(final_confusion_matrix)):
            if i==j:
                tag_acc = final_confusion_matrix[i][j] / final_confusion_matrix[i].sum(axis=0)
                list_tag_accuracy.append(tag_acc)
    return(final_accuracy_of_tagging_words, final_accuracy_of_tagging_sentences, final_confusion_matrix, list_tag_accuracy )
#%% 
sentences = []
# To use original word form = 1, to use lemma form =2. Default is 2.
original_or_stem = 1
filename = r"Project (Application 1) (MetuSabanci Treebank).conll"
ta = []
sentences = read_file(filename, original_or_stem)
accuracy_of_tagging_words, accuracy_of_tagging_sentences, final_confusion_matrix, list_tag_accuracy  = evaluation(sentences, original_or_stem)
